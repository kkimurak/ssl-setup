...
function install_libraries() {
    ...
    case "$DISTRIBU" in
        "fedora" )
            ...
            sudo dnf -y install git boost-devel clang cmake eigen3 libtool make ninja-build protobuf-devel automake gcc gcc-c++ kernel-devel qt-devel mesa-libGL-devel mesa-libGLU-devel protobuf-compiler ode ode-devel gtkmm24-devel libjpeg libpng v4l-utils libdc1394 libdc1394-devel zlib || echo "Failed to instlal some packages."
        ...
        "ubuntu" )
            ...
            # install most of required packages for Robocup-SSL official tools (without Autoref)
            sudo apt-get -y install git build-essential cmake libqt4-dev libgl1-mesa-dev libglu1-mesa-dev libprotobuf-dev protobuf-compiler libode-dev libboost-all-dev g++ libeigen3-dev libdc1394-22 libdc1394-22-dev libv4l-0 zlib1g-dev libgtkmm-2.4-dev || echo "Failed to install some packages"
